...
def change_bert_forward(bert):
    ...
    def _enc_forward(
        ...
        for i, layer_module in enumerate(enc_self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states,
                attention_mask,
                ctx_hidden_states=ctx_hidden_states[i] if ctx_hidden_states is not None else None,
            )
    ...
    def bert_forward(
        ...
        if ctx_input_ids is not None:
            ...
            ctx_sequence_output = ctx_encoder_outputs["hidden_states"][1:]
...
class BERTPlugDV2(nn.Module):
    def __init__(self, t5path="bert-base-uncased"):
        super().__init__()
        self.model = BertModel.from_pretrained(t5path)
        change_bert_forward(self.model)
        self.output = nn.Linear(self.model.config.hidden_size, 2)
...
if __name__ =="__main__":
    ...
    # )
    model = BERTPlugDV2("bert-base-uncased")
